...
def add_issue_to_parent(obj, jira_project, issue):
    ...
    if j_issue.jira_id:
        ...
        try:
            ...
            # if already Jira issue doesn't have parent, then error is returned; so ignore
            logger.debug('error updating parent issues for %s', obj.id)
            return False

# process jira form with parent information
def process_jira_form_parent_issue(engagement, parent_name):
    ...
    if jira_project.enable_parent_issue_linking and not jira_project.enable_engagement_epic_mapping:
        if parent_name:
            ...
            try:
                ...
                if engagement.has_jira_issue:
                    logger.debug("engagement already has_jira_issue")
                    ...
                    engagement.jira_issue = j_issue
                    engagement.save()
                else:
                    logger.debug("create jira issue for the engagement")
                    j_issue = JIRA_Issue(
            ...
            if engagement.has_jira_issue:
                logger.debug("no parent name provided, update jira issue details accordingly")
                j_issue = engagement.jira_issue
                j_issue.jira_key = ''
                j_issue.jira_id = ''
                j_issue.save()
    else:
        logger.debug("enable_parent_issue_linking is false or enable_engagement_epic_mapping is enabled")
...
def add_jira_issue(obj, *args, **kwargs):
    ...
    try:
        ...
        if epic_name_field in meta['projects'][0]['issuetypes'][0]['fields']:
            ...
            if not jira_project.enable_parent_issue_linking and not jira_project.component:
        ...
        logger.info('sending fields to JIRA: %s', fields)
        ...
        # check if parent issue needs to be updated
...
def process_jira_epic_form(request, engagement=None):
    ...
    if jira_project:
        if jira_epic_form.is_valid():
            ...
            process_jira_form_parent_issue(engagement, parent_name)
